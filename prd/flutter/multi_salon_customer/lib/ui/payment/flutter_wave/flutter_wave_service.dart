...
class FlutterWaveService {
  ...
  Future handlePaymentSuccess() async {
    ...
    if (bookingScreenController.createBookingCategory?.status == true) {
      ...
      for (var i = 0;
          i <
              (categoryDetailController.getServiceCategory?.services?.length ??
                  0);
          i++) {
      ...
      for (var i = 0;
          i <
              (homeScreenController.getExpertCategory?.data?.services?.length ??
                  0);
          i++) {
  ...
  void handlePaymentInitialization(BuildContext context) async {
    ...
    final Flutterwave flutterwave = Flutterwave(
      ...
      currency:
          splashController.settingCategory?.setting?.currencyName ?? "USD",
      redirectUrl: "https://www.google.com/",
      txRef: DateTime.now().microsecond.toString(),
      amount: bookingScreenController.totalPrice.toString(),
      customer: customer,
    ...
    log("Flutter Wave ----------- ");
    Utils.showToast(context, "Payment ${response.status.toString()}");
