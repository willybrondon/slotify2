...
class EasyDayWidget extends StatelessWidget {
  ...
  BoxDecoration get _inactiveDayDecoration {
    ...
    return _inactiveDayStyle.decoration ?? easyDayProps.inactiveDayDecoration ?? _buildDayDefaultDecoration(activeDayColor);
  }

  BoxDecoration get _activeDayDecoration {
    //TODO: remove `easyDayProps.activeDayDecoration` because it is deprecated.
    return _activeDayStyle.decoration ?? easyDayProps.activeDayDecoration ?? _buildDayDefaultDecoration(activeDayColor);
  }

  BoxDecoration get _disabledDayDecoration {
    return _disabledDayStyle.decoration ?? _buildDayDefaultDecoration(activeDayColor);
  }
  ...
  BoxDecoration _buildDayDefaultDecoration(Color backgroundColor) {
    ...
    final isBackgroundHighlight = easyDayProps.todayHighlightStyle == TodayHighlightStyle.withBackground;
    // Check if the dayHighlightStyle is `withBorder` it returns true otherwise it returns false.

    final isBorderHighlight = easyDayProps.todayHighlightStyle == TodayHighlightStyle.withBorder;
    // Indicating that today's date should be highlighted or not.

    final shouldHighlightToday = easyDayProps.todayHighlightStyle != TodayHighlightStyle.none;

    // Get the highlight color for today or use a default color if it should be highlighted
    final todayHighlightColor =
        easyDayProps.todayHighlightColor ?? (isBackgroundHighlight ? backgroundColor.withOpacity(0.2) : backgroundColor);
    ...
    return BoxDecoration(
      ...
      color: isSelected ? backgroundColor : (hasBackgroundHighlight ? todayHighlightColor : null),
      // color: isSelected
      //     ? backgroundColor
      ...
      border: hideBorder
          ? null
          : Border.all(
              color: hasBorderHighlight ? todayHighlightColor : easyDayProps.borderColor,
  ...
  BorderRadius _dayBorderRadius(bool isToday) {
    ...
    final activeBorderRadius = _activeDayStyle.borderRadius ?? easyDayProps.activeBorderRadius;
    //TODO: remove `easyDayProps.inactiveBorderRadius` because it is deprecated.
    final inactiveBorderRadius = _inactiveDayStyle.borderRadius ?? easyDayProps.inactiveBorderRadius;
    final disabledBorderRadius = _disabledDayStyle.borderRadius ?? EasyConstants.dayWidgetBorderRadius;
  ...
  DayInfoText _buildMonth(bool isToday) {
    ...
    final inactiveMothStrStyle =
        _inactiveDayStyle.monthStrStyle ?? easyDayProps.inactiveMothStrStyle ?? EasyTextStyles.monthStrStyle;
    //TODO: remove `easyDayProps.todayMonthStrStyle` because it is deprecated.
    final todayMonthStrStyle = _todayStyle.monthStrStyle ?? easyDayProps.todayMonthStrStyle ?? EasyTextStyles.monthStrStyle;
  ...
  DayInfoText _buildDayNumber(bool isToday) {
    ...
    final inactiveDayNumStyle = _inactiveDayStyle.dayNumStyle ?? easyDayProps.inactiveDayNumStyle ?? EasyTextStyles.dayNumStyle;
    //TODO: remove `easyDayProps.todayNumStyle` because it is deprecated.
    final todayNumStyle = _todayStyle.dayNumStyle ?? easyDayProps.todayNumStyle ?? EasyTextStyles.dayNumStyle;
  ...
  DayInfoText _buildDayString(bool isToday) {
    ...
    final inactiveDayStrStyle = _inactiveDayStyle.dayStrStyle ?? easyDayProps.inactiveDayStrStyle ?? EasyTextStyles.dayStrStyle;
    //TODO: remove `easyDayProps.disabledDayStrStyle` because it is deprecated.
    final todayStrStyle = _todayStyle.dayStrStyle ?? easyDayProps.todayStrStyle ?? EasyTextStyles.dayStrStyle;
  ...
  Widget _buildDayStructure({
    ...
    bool isNotCompleteDate = (structure == DayStructure.dayNumDayStr || structure == DayStructure.dayStrDayNum);
    return isLandScape
        ? Row(
  ...
  MainAxisAlignment _dayItemsMainAxisAlignment(bool isNotCompleteDate) {
    return isNotCompleteDate ? MainAxisAlignment.center : MainAxisAlignment.spaceEvenly;
  }
}
