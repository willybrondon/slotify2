...
class FcmSyncService {
  ...
  static Future<void> syncUserTokenIfLoggedIn() async {
    final isLoggedIn = Constant.storage.read<bool>('isLogIn') == true;
    final userId = Constant.storage.read<String>('userId')?.toString();
    if (!isLoggedIn || userId == null || userId.isEmpty) return;

    final token = FcmTokenUtil.current.isNotEmpty ? FcmTokenUtil.current : await refreshToken();
    if (token.isEmpty) return;

    try {
      final url = Uri.parse('${ApiConstant.BASE_URL}user/fcmToken');
      ...
      log('[FCM] user sync status=${response.statusCode} body=${response.body}');
    } catch (e) {
      log('[FCM] user sync error: $e');
    }
  }
