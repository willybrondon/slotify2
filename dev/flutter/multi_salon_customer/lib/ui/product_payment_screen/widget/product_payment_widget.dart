          ...
          builder: (logic) {
            return Column(
              ...
              children: [
                ...
                if (logic.salonName?.isNotEmpty == true)
                  Padding(
                    padding: const EdgeInsets.only(bottom: 12),
                    child: Text(
                      '${"txtSoldBy".tr}: ${logic.salonName}',
                      style: TextStyle(
                ...
                if (logic.showSalonStripe) const ProductPaymentSalonStripeView(),
                if (logic.showWallet) ProductPaymentMyWalletView(),
                if (!logic.showSalonStripe && !logic.showWallet)
                  Padding(
                    padding: const EdgeInsets.symmetric(vertical: 20),
                    child: Text(
                      "desNoPaymentMethodAvailable".tr,
...
class ProductPaymentSalonStripeView extends StatelessWidget {
  const ProductPaymentSalonStripeView({super.key});
    ...
    return GetBuilder<ProductPaymentController>(
      ...
      builder: (logic) {
        return InkWell(
          ...
          onTap: () => logic.onSelectPaymentMethod("Stripe"),
          child: Container(
            height: 60,
            width: Get.width,
...
class ProductPaymentMyWalletView extends StatelessWidget {
  const ProductPaymentMyWalletView({super.key});
  ...
  Widget build(BuildContext context) {
    return GetBuilder<ProductPaymentController>(
      ...
      builder: (logic) {
        return InkWell(
          overlayColor: WidgetStatePropertyAll(AppColors.transparent),
          onTap: () => logic.onSelectPaymentMethod("wallet"),
          child: Container(
            height: 60,
            width: Get.width,
            padding: const EdgeInsets.only(left: 10, right: 5),
