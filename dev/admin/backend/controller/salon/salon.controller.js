  ...
  try {
    ...
    if (salon) {
      if (req.body.password != salon.password) {
        return res
          .status(200)
          .send({ status: false, message: "Oops ! Invalid Password!!" });
      }
      if (!salon.isActive) {
        return res.status(200).send({
          status: false,
          message:
            "Salon is blocked by admin!! Contact Admin from more details",
        });
      }
    } else {
      return res
        .status(200)
        .send({ status: false, message: "Salon Not Found!! Contact Admin" });
    }
...
exports.addServices = async (req, res) => {
  try {
    ...
    const existingService = salon.serviceIds.find(
      (s) => String(s.id._id) === String(serviceId)
    );
...
exports.removeService = async (req, res) => {
  try {
    ...
    const serviceIndex = salon.serviceIds.findIndex(
      (service) => service.id._id.toString() === req.query.serviceId
    );

    if (serviceIndex === -1) {
      return res
        .status(200)
        .json({ status: false, message: "Service ID not found in salon" });
    }
    ...
    for (const expert of experts) {
      expert.serviceId = expert.serviceId.filter(
        (id) => id.toString() !== req.query.serviceId
      );
      await expert.save();
    }
  ...
  try {
    if (
      ...
      ![
        ...
        "Saturday",
        "Sunday",
      ].includes(req.query.day)
    ) {
      return res
        .status(200)
        .json({ status: false, message: "Invalid day provided" });
    }
    ...
    if (!salon) {
      return res
        .status(200)
        .json({ status: false, message: "Salon does not found" });
    }
    ...
    // if (req.body.openTime) salonDay.openTime = req.body.openTime;
    // if (req.body.closedTime) salonDay.closedTime = req.body.closedTime;
    // salonDay.breakStartTime = req.body.breakStartTime
    //   ? req.body.breakStartTime
    //   : salonDay.breakStartTime;

    // salonDay.breakEndTime = req.body.breakEndTime
    //   ? req.body.breakEndTime
    //   : salonDay.breakEndTime;


    // if (salonDay.isBreak) {
    //   if (salonDay.breakStartTime === "" || salonDay.breakEndTime === "") {
    //     return res
    //       .status(200)
    //       .json({
    //         status: false,
    //         message: "Break start-time  and break end-time is required if salon have break",
    //       });
    //   }
    // }
...
exports.updateSalonPassword = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .send({ status: true, message: "password updated", salon });
  } catch (error) {
    console.log(error);
    return res
      .status(500)
...
exports.getCurrency = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .send({ status: true, data: setting, message: "currency found" });
  } catch (error) {
    console.log(error);
    return res
      .status(500)
