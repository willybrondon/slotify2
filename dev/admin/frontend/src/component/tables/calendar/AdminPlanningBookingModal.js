...
import {
  createAdminPlanningBooking,
  searchAdminPlanningClients,
} from "../../../redux/slice/teamScheduleSlice";
import { Success } from "../../api/toastServices";

const AdminPlanningBookingModal = ({ salonId, slot, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const res = await dispatch(searchAdminPlanningClients(clientSearch)).unwrap();
  ...
  const services = useMemo(() => salonDetail?.serviceIds || [], [salonDetail]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!serviceIds.length || !salonId) return;
    setSubmitting(true);
    try {
      await dispatch(
        createAdminPlanningBooking({
  ...
  return (
    <div className="sq-planning-modal-backdrop" onClick={onClose} role="presentation">
      <div className="sq-planning-modal" onClick={(e) => e.stopPropagation()} role="dialog">
        ...
        <form onSubmit={handleSubmit} className="sq-planning-modal__body">
          ...
          <div className="sq-planning-modal__footer">
            ...
            <button type="submit" className="btn sq-btn-primary" disabled={submitting || !serviceIds.length}>
...
export default AdminPlanningBookingModal;
