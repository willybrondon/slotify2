...
import {
  cancelAdminPlanningBooking,
  fetchAdminPlanningBookingDetail,
  updateAdminPlanningBookingServices,
} from "../../../redux/slice/teamScheduleSlice";
import { getSalonDetail } from "../../../redux/slice/salonSlice";
...
const AdminPlanningBookingDetailModal = ({ salonId, bookingId, onClose, onUpdated }) => {
  ...
  const loadBooking = useCallback(() => {
    if (!bookingId || !salonId) return;
    setLoading(true);
    dispatch(fetchAdminPlanningBookingDetail({ salonId, bookingId }))
      .unwrap()
      .then((res) => {
  ...
  }, [bookingId, salonId, dispatch]);

  useEffect(() => {
    loadBooking();
    if (salonId) {
      dispatch(getSalonDetail(salonId))
        .unwrap()
        .then((res) => setSalonDetail(res?.salon || null))
        .catch(() => setSalonDetail(null));
    }
  }, [loadBooking, salonId, dispatch]);
  ...
  const availableServices = useMemo(() => {
    const list = salonDetail?.serviceIds || [];
    ...
    try {
      await dispatch(
        cancelAdminPlanningBooking({
          salonId,
          bookingId: booking._id,
          reason: cancelReason || "Annulé depuis le planning admin",
        })
      ).unwrap();
  ...
  return (
    <div className="sq-planning-modal-backdrop" onClick={onClose} role="presentation">
      <div className="sq-planning-modal sq-booking-detail" onClick={(e) => e.stopPropagation()}>
          ...
          {!loading && booking && (
            <>
              <div className="sq-detail-grid">
                ...
                <div className="sq-detail-item">
                  <span>Horaire</span>
                  <strong>{booking.startTime || booking.time?.[0] || "—"}</strong>
                </div>
                <div className="sq-detail-item">
                  <span>Durée</span>
        ...
        <div className="sq-planning-modal__footer">
          <button type="button" className="btn btn-light" onClick={onClose}>Fermer</button>
        </div>
      </div>
    </div>
  );
...
export default AdminPlanningBookingDetailModal;
