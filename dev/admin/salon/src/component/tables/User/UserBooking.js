...
import { getUserBookings } from "../../../redux/slice/userSlice";
...
const UserBooking = () => {
  ...
  const { booking, total } = useSelector((state) => state.user);
  ...
  useEffect(() => {
    dispatch(getUserBookings({ ...payload, command: true }));
