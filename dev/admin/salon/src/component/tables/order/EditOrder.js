...
import { getOrders, updateOrderOutOfStatus, updateOrderStatus } from "../../../redux/slice/orderSlice";
...
const EditOrder = ({ page, rowsPerPage, type }) => {
    ...
    const filteredOrderType = orderType.filter((option) => {
        if (dialogueData?.item?.status === "Pending") {
            return true; // Show all options for "Pending"
        }
        if (dialogueData?.item?.status === "Confirmed") {
            return option.value === "Confirmed" || option.value === "Out Of Delivery";
        }
        if (dialogueData?.item?.status === "Out Of Delivery") {
            return option.value === "Out Of Delivery" || option.value === "Delivered";
        }
        return true; // Default case, show all options
    });
    useEffect(() => {
        setStatusData(dialogueData?.item?.status || ui.labels.selectStatus);
    }, [dialogueData]);


    console.log("dialogueData", dialogueData, dialogueMainData)
    ...
    const handleSubmit = (e) => {
        ...
        if (statusData === ui.labels.selectStatus || !statusData) {
            ...
            dispatch(updateOrderOutOfStatus({
                ...
                itemId: dialogueData?.item?._id,
                status: statusData,
                userId: dialogueMainData?.userId,
                data: {
                            ...
                            <div className="col-xl-5 col-md-8 col-11">
                                <div className="mainDiaogBox">
                                    ...
                                    <form onSubmit={handleSubmit}>
                                        ...
                                        {statusData === "Out Of Delivery" && dialogueData?.item?.status !== "Out Of Delivery" && (
