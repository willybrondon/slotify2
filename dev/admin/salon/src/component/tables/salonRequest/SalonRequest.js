...
import { getSalonRequest } from "../../../redux/slice/salonRequestSlice";
import image from "../../../assets/images/image.png";
import { openDialog } from "../../../redux/slice/dialogueSlice";
import SalonRequestDialogue from "./SalonRequestDialogue";

export const SalonRequest = () => {
  const { salonRequest } = useSelector((state) => state.salonRequest);
    ...
    },
    {
      ...
      Cell: ({ row }) => (
        <div className="userProfile">
          <img
            src={!row?.image?.includes("\\") ? row?.image : image}
            alt="image"
      ...
      Cell: ({ row }) => (
        <span>
          <button
            ...
            onClick={() =>
              dispatch(openDialog({ type: "salonRequest", data: row }))
            }
          >
            <svg
              width="26"
  ...
  return (
    <>
      {dialogueType == "salonRequest" && <SalonRequestDialogue />}
      <Table data={data} mapData={salonRequestTable} />
    </>
  );
};
